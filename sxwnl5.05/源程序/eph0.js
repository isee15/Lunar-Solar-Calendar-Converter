//=============================IE6��toFixed() Bug===================================
//==================================================================================
Number.prototype.toFixed = function (m) { //IE6.0��toFixed()��Bug,������д
    var n = this, f = '', p = Math.pow(10, m); //pΪ10������λ��;
    if (n < 0) n = -n, f = '-';   //�Ѹ���תΪ����
    var a = Math.floor(n), b = n - a; //����������С��
    b = Math.round(b * p);      //��λ����������
    if (b >= p) a++, b -= p;         //��λ
    if (m) b = '.' + (p + b + '').substr(1, m); //С�����߲�0
    else b = '';
    return f + a + b;
};
//=================================���ĳ���=========================================
//==================================================================================
var cs_rEar = 6378.1366; //�������뾶(ǧ��)
var cs_rEarA = 0.99834 * cs_rEar; //ƽ��뾶
var cs_ba = 0.99664719; //���򼫳�뾶��
var cs_ba2 = cs_ba * cs_ba; //���򼫳�뾶�ȵ�ƽ��
var cs_AU = 1.49597870691e8; //���ĵ�λ����(ǧ��)
var cs_sinP = cs_rEar / cs_AU;   //sin(̫���Ӳ�)
var cs_PI = Math.asin(cs_sinP); //̫���Ӳ�
var cs_GS = 299792.458; //����(����/��)
var cs_Agx = cs_AU / cs_GS / 86400 / 36525; //ÿ���ĵ�λ�Ĺ���ʱ��(��������)
var cs_xxHH = new Array(116, 584, 780, 399, 378, 370, 367, 367); //���ǻ������
var xxName = new Array('����', 'ˮ��', '����', '����', 'ľ��', '����', '������', '������', 'ڤ����');
var rad = 180 * 3600 / Math.PI; //ÿ���ȵĽ�����
var radd = 180 / Math.PI; //ÿ���ȵĶ���
var pi2 = Math.PI * 2;
var pi_2 = Math.PI / 2;
var J2000 = 2451545;

var cs_k = 0.2725076; //���������İ뾶��(���ڰ�Ӱ����)
var cs_k2 = 0.2722810; //���������İ뾶��(���ڱ�Ӱ����)
var cs_k0 = 109.1222;  //̫�������İ뾶��(��Ӧ959.64)
var cs_sMoon = cs_k * cs_rEar * 1.0000036 * rad;  //���������Ӱ뾶����
var cs_sMoon2 = cs_k2 * cs_rEar * 1.0000036 * rad; //���������Ӱ뾶����
var cs_sSun = 959.64; //����̫���Ӱ뾶����
...
//=================================�Ƕȸ�ʽ��=======================================
//==================================================================================
function rad2strE(d, tim, ext) { //������תΪ�ִ�,extΪС����λ��
    //tim=0�����ʽʾ��: -23��59' 48.23"
    //tim=1�����ʽʾ��:  18h 29m 44.52s
    var s = " ";
    var w1 = "��", w2 = "'", w3 = '"';
    if (d < 0)  d = -d, s = '-';
    if (tim) {
...
function m2fm(v, fx, fs) {//��תΪ����,fxΪС���λ��,fsΪ1תΪ"����"��ʽ����תΪ"�Ƕȷ���"��ʽ
    var gn = '';
    if (v < 0) v = -v, gn = '-';
    var f = Math.floor(v / 60), m = v - f * 60;
    if (!fs)   return gn + f + "'" + m.toFixed(fx) + '"';
    if (fs == 1) return gn + f + '��' + m.toFixed(fx) + '��';
    if (fs == 2) return gn + f + 'm' + m.toFixed(fx) + 's';
}


function str2rad(s, f) { //��ת����, f=1��ʾ�����sΪʱ����
    var fh = 1;
    f = f ? 15 : 1;
    if (s.indexOf('-') != -1) fh = -1;
    s = s.replace(/h|m|s|(-)|(��)|\'|\"/g, ' ');
    s = s.replace(/ +/g, ' ');   //����ո�ϲ�Ϊ1���ո�
    s = s.replace(/(^\s*)|(\s*$)/g, ''); //ȥ��ǰ��ո�
    s = s.split(' ');
    return fh * (s[0] * 3600 + s[1] * 60 + s[2] * 1) / rad * f;
}

/*
 function str2rad(s,f){ //��ת����, f=1��ʾ�����sΪʱ���롣ΪҲ��flash as2�������Բ���replace
...
function rad2mrad(v) { //�Գ���0-2PI�ĽǶ�תΪ0-2PI
    v = v % (2 * Math.PI);
    if (v < 0) return v + 2 * Math.PI;
    return v;
}
function rad2rrad(v) {//�Գ���-PI��PI�ĽǶ�תΪ-PI��PI
    v = v % (2 * Math.PI);
    if (v <= -Math.PI) return v + 2 * Math.PI;
    if (v > Math.PI) return v - 2 * Math.PI;
    return v;
}
function mod2(a, b) { //�ٽ�����(a����������b���ľ���)
    var c = a / b;
    c -= Math.floor(c);
    if (c > 0.5) c -= 1;
    return c * b;
...
function CD2DP(z, L, fa, gst) { //������תΪ��ƽ���
    var a = new Array(z[0] + Math.PI / 2 - gst - L, z[1], z[2]);  //ת������ڵ�ƽ����ֵ�ĳ�����
    a = llrConv(a, Math.PI / 2 - fa);
    a[0] = rad2mrad(Math.PI / 2 - a[0]);
    return a;
}
function j1_j2(J1, W1, J2, W2) { //��ǶȲ�
...
function shiChaJ(gst, L, fa, J, W) { //�Ӳ��(�����Ӳ�)
    var H = gst + L - J; //�����ʱ��
...
function dt_calc(y) { //��������ʱ��ԭ��ʱ֮��,������
    var y0 = dt_at[dt_at.length - 2]; //�������һ��
    var t0 = dt_at[dt_at.length - 1]; //�������һ���deltatT
    if (y >= y0) {
        var jsd = 31; //sjd��y1��֮��ļ��ٶȹ��ơ���ʿ�����jsd=31,NASA��վjsd=32,skmap��jsd=29
        if (y > y0 + 100) return dt_ext(y, jsd);
        var v = dt_ext(y, jsd);       //������������
        var dv = dt_ext(y0, jsd) - t0;  //ye��Ķ���������te�Ĳ�
...
var JD = { //����Ԫ��
    ...
    nnweek: function (y, m, n, w) { //��y��m�µĵ�n������w����������
        var jd = JD.JD(y, m, 1.5); //����������
        var w0 = (jd + 1 + 7000000) % 7;       //���׵�����
        var r = jd - w0 + 7 * n + w;    //jd-w0+7*n�Ǻ�n������0,�����±��µ�һ�е�������(����������һ��)����w��Ϊ��n������w
        if (w >= w0) r -= 7; //��1������w���������ϸ���,��ɶ���1��,���Կ��Ǽ�1��
        if (n == 5) {
            m++;
            if (m > 12) m = 1, y++;   //�¸���
            if (r >= JD.JD(y, m, 1.5)) r -= 7; //r�ܵ��¸������1��
        }
        return r;
    }
};
...
//=================================������=========================================
//==================================================================================
var preceTab_IAU1976 = new Array( //IAU1976����
    ...
    0, 47.0028, -0.03301, +0.000057, //pi(�Ե���),���P=sin(pi)*sin(II)
    629554.886, -869.8192, +0.03666, -0.001504, //II(�Ե���),���Q=sin(pi)*cos(II)
...
var preceTab_IAU2000 = new Array( //IAU2000����
    ...
    0, 47.0028, -0.03301, +0.000057, 0, 0, //pi(�Ե���)
    629554.886, -869.8192, +0.03666, -0.001504, 0, 0, //II(�Ե���)
...
function prece(t, sc, mx) { //t������������,sc����������,mx��ģ��
    var i, tn = 1, c = 0, p, n;
...
//=================================�����ת=========================================
//==================================================================================
function CDllr_J2D(t, llr, mx) { //J2000���תDate���
...
function CDllr_D2J(t, llr, mx) { //Date���תJ2000���
    var Z = -prece(t, 'z', mx) + llr[0];
    var z = -prece(t, 'Z', mx);
    var th = -prece(t, 'th', mx);
    var cosW = cos(llr[1]), cosH = cos(th);
...
function HDllr_J2D(t, llr, mx) {//�Ƶ��������_J2000תDate�ֵ�,tΪ����������
    //J2000�Ƶ���ת��Date�Ƶ�(���������),Ҳ��ֱ��������������ת�������,�����ǽӽ�Ϊ0ʱ���Ⱥܵ�
...
function HDllr_D2J(t, llr, mx) {//�Ƶ��������_Date�ֵ�תJ2000,tΪ����������
    var r = new Array(llr[0], llr[1], llr[2]);
    r = llrConv(r, prece(t, 'E', mx));
    r[0] += prece(t, 'x', mx);
    r = llrConv(r, -prece(t, 'w', mx));
...
function nutation(t, zq) { //�¶�����,t��J2000.0���������������,zq��ʾֻ�������ڴ���zq(��)����
    var t2 = t * t, t3 = t2 * t, t4 = t3 * t; //t�Ķ������Ĵη�
    ...
    return new Array(dL /= 10000000 * rad, dE /= 10000000 * rad); //����IAU2000B�¶�ֵ, dL�ǻƾ��¶�,dE�ǽ����¶�
}

function CDnutation(z, E, dL, dE) { //���������ྭ�¶�����γ�¶�
    var r = new Array(z[0], z[1], z[2]);
    r[0] += ( cos(E) + sin(E) * sin(z[0]) * tan(z[1]) ) * dL - cos(z[0]) * tan(z[1]) * dE; //�ྭ�¶�
    r[1] += sin(E) * cos(z[0]) * dL + sin(z[0]) * dE; //��γ�¶�
    r[0] = rad2mrad(r[0]);
    return r;
}


var nutB = new Array(//�о����¶������
...
function nutation2(t) { //�о����¶�����,t��������
    ...
    return [dL / 100 / rad, dE / 100 / rad]; //�ƾ��¶�,�����¶�
}
function nutationLon2(t) { //ֻ����ƾ��¶�
...
//=================================�Ӳ����=========================================
//==================================================================================
function parallax(z, H, fa, high) { //�Ӳ�����
                                    //z������,fa����γ��,Hʱ��,high����(ǧ��)
    var dw = 1;
    if (z[2] < 500) dw = cs_AU;
    z[2] *= dw;
    var r0, x0, y0, z0, f = cs_ba, u = Math.atan(f * Math.tan(fa)), g = z[0] + H;
    r0 = cs_rEar * Math.cos(u) + high * Math.cos(fa); //վ����ص����򾶵ĳ��ͶӰ����
    z0 = cs_rEar * Math.sin(u) * f + high * Math.sin(fa); //վ����ص����򾶵�����ͶӰ����
    x0 = r0 * Math.cos(g);
    y0 = r0 * Math.sin(g);
...
var XL0 = new Array(
//Dear����:J2000+-4ǧ�� �ƾ�0.1���� ��γ0.1���� ����0.1AU/10^6
    new Array(
        10000000000,//A�ı���
...
//Dmer����:J2000+-4ǧ�� �ƾ�0.2���� ��γ0.2���� ����0.2AU/10^6
    new Array(
        1000000000,//A�ı���
        20, 443, 710, 761, 791, 818, 824, 1043, 1106, 1142, 1169, 1190, 1196, 1550, 1742, 1781, 1808, 1823, 1823,//λ�������
...
//Dven����:J2000+-4ǧ�� �ƾ�0.2���� ��γ0.2���� ����0.2AU/10^6
    new Array(
        1000000000,//A�ı���
        20, 257, 374, 425, 437, 449, 458, 566, 629, 641, 653, 665, 668, 929, 1040, 1082, 1091, 1094, 1094,//λ�������
...
//Dmar����:J2000+-4ǧ�� �ƾ�0.5���� ��γ0.5���� ����1AU/10^6
    new Array(
        1000000000,//A�ı���
        20, 596, 1028, 1289, 1385, 1427, 1454, 1586, 1670, 1694, 1709, 1718, 1724, 2360, 2873, 3155, 3239, 3275, 3287,//λ�������
...
//Djup����:J2000+-4ǧ�� �ƾ�0.5���� ��γ0.5���� ����3AU/10^6
    new Array(
        100000000,//A�ı���
        20, 503, 863, 1256, 1451, 1529, 1550, 1676, 1802, 1910, 1964, 1988, 1991, 2513, 2945, 3482, 3761, 3896, 3923,//λ�������
...
//Dsat����:J2000+-4ǧ�� �ƾ�0.5���� ��γ0.5���� ����5AU/10^6
    new Array(
        100000000,//A�ı���
        20, 806, 1406, 1946, 2177, 2282, 2333, 2537, 2726, 2867, 2963, 3008, 3026, 4091, 5063, 5789, 6260, 6452, 6536,//λ�������
...
//Dura����:J2000+-4ǧ�� �ƾ�1���� ��γ1���� ����20AU/10^6
    new Array(
        100000000,//A�ı���
        20, 539, 836, 980, 1070, 1085, 1088, 1193, 1271, 1307, 1322, 1325, 1325, 2150, 2660, 2936, 3089, 3122, 3122,//λ�������
...
//Dnep����:J2000+-4ǧ�� �ƾ�1���� ��γ1���� ����40AU/10^6
    new Array(
        100000000,//A�ı���
        20, 188, 260, 281, 293, 299, 302, 359, 404, 419, 422, 425, 425, 638, 701, 743, 746, 746, 746,//λ�������
...
/******
 ...
 �����
 1.���Ե���N��2<N<10001����500 :2300
 2.ƫ����rnd��0<=rnd<=1����0.3 :0.2
 ���      ƽ��
 ��  0.468217  0.080642 (����)
 γ  0.346452  0.071540 (����)
 ��  0.724653  0.153055 (ǧ��)
...
function XL0_calc(xt, zn, t, n) { //xt����,zn����,t����������,n��������
    t /= 10; //תΪ����ǧ����
    var i, j, v = 0, tn = 1, c;
...
function pluto_coord(t) { //����ڤ����J2000ֱ�����
...
function p_coord(xt, t, n1, n2, n3) { //xt����,T����������,TD
    var z = new Array();
    if (xt < 8) {
...
function e_coord(t, n1, n2, n3) { //���ص������,tΪ������
    var re = new Array();
...
function XL1_calc(zn, t, n) { //��������
    ...
    if (zn == 0) {
        v += (3.81034409 + 8399.684730072 * t - 3.319e-05 * t2 + 3.11e-08 * t3 - 2.033e-10 * t4) * rad; //����ƽ�ƾ�(����)
        v += 5028.792262 * t + 1.1124406 * t2 + 0.00007699 * t3 - 0.000023479 * t4 - 0.0000000178 * t5;  //���(����)
        if (tx > 0) v += -0.866 + 1.43 * tx + 0.054 * tx * tx; //�Թ�Ԫ3000������Ԫ5000������,������С��10����
    }
...
function m_coord(t, n1, n2, n3) { //�����������,n1,n2,n3Ϊ�������ȡ������
    var re = new Array();
...
function gxc_sunLon(t) { //̫�����в�,t��������
    var v = -0.043126 + 628.301955 * t - 0.000002732 * t * t; //ƽ����
    var e = 0.016708634 - 0.000042037 * t - 0.0000001267 * t * t;
    return ( -20.49552 * (1 + e * Math.cos(v)) ) / rad; //�ƾ����в�
}
...
function pGST(T, dt) { //����T��2000�������������(UT),dt��deltatT(��),����Ҫ�󲻸�ʱdt��ȡֵΪ0
                       //���ظ�������ƽ����ʱ(�����ྭ�¶����Ƕ���ʽ����),��������������Ȧ��ƽ���������ĳྭ
    var t = (T + dt) / 36525, t2 = t * t, t3 = t2 * t, t4 = t3 * t;
    return pi2 * (0.7790572732640 + 1.00273781191135448 * T) //T��UT,��һ�е�t����ѧʱ(������)
        + (0.014506 + 4612.15739966 * t + 1.39667721 * t2 - 0.00009344 * t3 + 0.00001882 * t4) / rad;
}
function pGST2(jd) { //������ѧʱJ2000���������ƽ����ʱ
    var dt = dt_T(jd);
    return pGST(jd - dt, dt);
}

function sunShengJ(jd, L, fa, sj) { //̫������㡣jd������(��ӽ�L����ƽ��UT)��L���?�ȣ�fa����γ�ȣ�sj=-1��,sj=1��
    var i;
    jd = floor(jd + 0.5) - L / pi2;
    for (i = 0; i < 2; i++) {
        var T = jd / 36525, E = (84381.4060 - 46.836769 * T) / rad; //�Ƴཻ��
        var t = T + (32 * (T + 1.8) * (T + 1.8) - 20) / 86400 / 36525;  //������������,��ѧʱ
        ...
        var sinJ = sin(J), cosJ = cos(J); //̫���ƾ��Լ����������ֵ
        var gst = (0.7790572732640 + 1.00273781191135448 * jd) * pi2 + (0.014506 + 4612.15739966 * T + 1.39667721 * T * T) / rad; //����ʱ(����Ȧλ��)
        var A = atan2(sinJ * cos(E), cosJ);  //̫���ྭ
        var D = asin(sin(E) * sinJ);        //̫����γ
        var cosH0 = (sin(-50 * 60 / rad) - sin(fa) * sin(D) ) / ( cos(fa) * cos(D) );
        if (abs(cosH0) >= 1) return 0; //̫���ڵ�ƽ���ϵ�cos(ʱ��)����
        jd += rad2rrad(sj * acos(cosH0) - (gst + L - A)) / 6.28; //(��ʱ��-̫��ʱ��)/̫���ٶ�
    }
    return jd; //���ظ�������UT
}


function pty_zty(t) { //ʱ�����(�߾���),t��ѧʱ����������
    var t2 = t * t, t3 = t2 * t, t4 = t3 * t, t5 = t4 * t;
    ...
    dL = -17.2 * Math.sin(2.1824 - 33.75705 * t) / rad; //�ƾ���
...
function pty_zty2(t) { //ʱ�����(�;���),���Լ��1������,t��ѧʱ����������
...
//=========================
    ...
    /****
     MS_aLon_t1:function(W){ //��֪�����ӻƾ�����ʱ��,���ٵ;���,������40��
  var t,v = 7771.37714500204;
  t  = ( W + 1.08472               )/v;
  t += ( W - this.MS_aLon(t, 3, 3) )/v;  v=this.M_v(t)-this.E_v(t);  //v�ľ���0.5%�����ԭ��
  t += ( W - this.MS_aLon(t,50,20) )/v;
  return t;
},
     S_aLon_t1:function(W){ //��֪̫���ӻƾ�����ʱ��,���ٵ;���,���������50��,ƽ�����15��
  var t,v= 628.3319653318;
  ...
  return t;
     ...
     ****/
    MS_aLon_t2: function (W) { //��֪�����ӻƾ�����ʱ��,���ٵ;���,������600��(ֻ�����˼�ǧ��)
        var t, v = 7771.37714500204;
        t = ( W + 1.08472 ) / v;
        var L, t2 = t * t;
    ...
    S_aLon_t2: function (W) { //��֪̫���ӻƾ�����ʱ��,���ٵ;���,���������600��
    ...
    moonIll: function (t) { //�������������ֵı���
        var t2 = t * t, t3 = t2 * t, t4 = t3 * t;
        var D, M, m, a, dm = Math.PI / 180;
        D = (297.8502042 + 445267.1115168 * t - 0.0016300 * t2 + t3 / 545868 - t4 / 113065000) * dm; //����ƽ���
        M = (357.5291092 + 35999.0502909 * t - 0.0001536 * t2 + t3 / 24490000             ) * dm; //̫��ƽ���
        m = (134.9634114 + 477198.8676313 * t + 0.0089970 * t2 + t3 / 69699 - t4 / 14712000  ) * dm; //����ƽ���
...
function dituJM(p, Jb, Wb) { //��ͼ����,p��ͼ��,Jb���ȵı���,Wbγ�ȱ���
    p = p.split('');
...
var ditu1 = '' //��Сͼ���С4200*2100���Ӧ360��*180��
            //��ŷ�Ǻ���
...
var touY = {
    MollY: new Array(),
    MollCZ: function (W) { //��ֵ����Ħ������ͶӰ��yֵ��W����pi_2֮ǰ
        var i, N = 100, B = this.MollY, f = 1;
        if (W < 0) W = -W, f = -1;
        if (W > pi_2 - 1E-10) return f;

        if (!B.length) { //������ֵ��,��NΪ100ʱ������Ϊ10^-7������80�ȵĸ߶�γ��Ƚ�Ϊ10^-6��88��Ϊ10^-5
            for (i = 0; i < N; i++) {
    ...
    toxy0: function (J, W, a) { //ƽ����Ͷ
        J -= this.J0 + pi_2; //ͼ�еľ��������򶫲���
    ...
    toxy1: function (J, W, a) {//б��Ⱦ෽λͶӰ
        ...
        a.x = a.y = 0, a.z = -1;
        if (B > Math.PI - 0.1) return; //�ӽ�180��ʧ��̫�󣬲��ؼ���
        B = B / Math.PI;
        a.x = B * cos(L), a.y = B * sin(L), a.z = 1;
    },
    toxy2: function (J, W, a) {//б��Ȼ�λͶӰ
        J -= this.J0;
        var cosJ = Math.cos(J), sinJ = Math.sin(J);
        var cosW = Math.cos(W), sinW = Math.sin(W);
        var L = Math.atan2(this.sinE0 * sinW - this.cosE0 * cosW * cosJ, sinJ * cosW);
    ...
    toxy3: function (J, W, a) {//б��ȽǷ�λͶӰ
        ...
        a.x = a.y = 0, a.z = -1;
        if (B > Math.PI - 1) return; //�ӽ�180��ʧ��̫�󣬲��ؼ���
        B = Math.tan(B / 2);
        a.x = B * Math.cos(L), a.y = B * Math.sin(L), a.z = 1;

    },
    toxy4: function (J, W, a) { //Ħ������
        W = rad2rrad(W);
    ...
    toxy7: function (J, W, a) { //��Բ׶
        J = rad2rrad(J - this.J0);
        var C = Math.cos(W); //γ�߳���
        var v = Math.tan(W); //γ������
        if (abs(W) > 0.002) {
            var s = C * J * v, l = 1 / v;
            a.x = l * Math.sin(s);
    ...
    toxy8: function (J, W, a) { //�й����ͶӰ
        J = rad2rrad(J - this.J0);
        var C = Math.cos(W * 2 / 3);     //γ�߳���
        var v = Math.sin(W * 2 / 3) / 5.3; //γ������
    ...
    setlx: function (lx, J0, W0, jb) {//��������, jbΪ�ֲ���ʾ����
    ...
    lineArr: function (d) { //������
        var p = new Array(), k = 0;
        var i, h, G = new Object(); //hΪ��ͷ���
        var lx, ly, dd = 0;         //ĳ���ߵ����һ�����ߵ�(lx,ly)��ddΪ����״̬
        if (d[0] != 1e7) h = 1;        //����moveto��ͷ������
        ...
        for (i = 0; i < d.length; i++) {
            ...
            i++; //��flash����ôдthis.toxy(d[i++],d[i],G);������ȷ����
            ...
            if (this.tylx == 4) { //Ħ�ϵ�ͼͶӰ
                if (abs(lx - G.x) > 1 / 3) h = 1; //��ֹ����ˮƽ��ɨ��(��ͼ���߽��и�ʱ���д�����)
            }
            if (this.tylx == 5 || this.tylx == 6) { //����Ⱦ��Ƚ�Բ��
                if (abs(lx - G.x) > 1 / 3 || abs(ly - G.y) > 1 / 3) h = 1; //��ֹ����ˮƽ��ɨ��
            }
            if (this.tylx == 7 || this.tylx == 8 || this.tylx == 9) { //��Բ׶
                var cy = abs(ly - G.y), cx = abs(lx - G.x);
                if (cx > 1 / 3 || abs(G.y) > 1 / 2 && lx * G.x < 0) h = 1; //��ֹ����ˮƽ��ɨ��
            }
            if (h) p[k++] = 1e7, h = 0;
            p[k++] = G.x, p[k++] = G.y;
            lx = G.x, ly = G.y;
