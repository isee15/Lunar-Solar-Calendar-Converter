...
function getfiles(path, lx) {
  //取指定路径内的文件表,lx为folder时取子目录表
  var f = fso.GetFolder(path),
    fc;
  if (lx == "folder") fc = new Enumerator(f.SubFolders);
  else fc = new Enumerator(f.files);
  var s = new Array();
  for (; !fc.atEnd(); fc.moveNext()) s[s.length] = fc.item();
  return s;
}
...
function jsCompress(s) {
  ...
  for (i = 0; i < s.length; i++) {
    ...
    if (c == "." && !cf && s.substr(i, 10) == ".replace(/") {
      //引号常量之外(即代码之中)的replace函数不压缩
...
for (i = 0; i < fs.length; i++) {
  if (fs[i] == "") {
    WScript.Echo("源js文件名称无效");
    getLine();
    WScript.Quit();
  }
  if (!fexists(ph + fs[i])) {
    WScript.Echo(fs[i] + " 文件不存在!");
    getLine();
    WScript.Quit();
  }
}
...
if (p1 >= 0 && p2 >= 0)
  s2 = s2.substr(0, p1) + s + s2.substr(p2, s2.length - p2);
////写目标文件(合并文件)
else s2 = s + s2;
...
WScript.Echo("目标文件：" + ph + fs + " 合并完成,任意键继续...");
getLine();
