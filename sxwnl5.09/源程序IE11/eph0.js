...
var cs_k2 = 0.272281; //月亮与地球的半径比(用于本影计算)
...
var preceTab_IAU1976 = new Array( //IAU1976岁差表
  ...
  47.0028,
  -0.03301,
  +0.000057, //pi(自导出),根据P=sin(pi)*sin(II)
  629554.886,
  -869.8192,
  +0.03666,
  -0.001504, //II(自导出),根据Q=sin(pi)*cos(II)
  0,
  5029.0966,
  +1.11113,
  +0.000006, //p
...
function nutation(t, zq) {
  ...
  for (i = 0; i < 77 * 11; i += 11) {
    //周期项取和计算
    c =
      (B[i] * l + B[i + 1] * l1 + B[i + 2] * F + B[i + 3] * D + B[i + 4] * Om) /
      rad;
    if (zq) {
      //只算周期大于zq天的项
      q =
...
function CDnutation(z, E, dL, dE) {
  ...
  r[0] +=
    (cos(E) + sin(E) * sin(z[0]) * tan(z[1])) * dL - cos(z[0]) * tan(z[1]) * dE; //赤经章动
...
function pGST(T, dt) {
  ...
  var t = (T + dt) / 36525,
    ...
    t4 = t3 * t;
  return (
    pi2 * (0.779057273264 + 1.00273781191135448 * T) + //T是UT,下一行的t是力学时(世纪数)
    (0.014506 +
      4612.15739966 * t +
      1.39667721 * t2 -
      0.00009344 * t3 +
...
function sunShengJ(jd, L, fa, sj) {
  ...
  for (i = 0; i < 2; i++) {
    ...
    var J =
      ...
      10000000;
    var sinJ = sin(J),
      cosJ = cos(J); //太阳黄经以及它的正余弦值
    var gst =
      (0.779057273264 + 1.00273781191135448 * jd) * pi2 +
...
var XL = {
  ...
  M_v: function (t) {
    //月球速度计算,传入世经数
    var v =
      8399.71 - 914 * Math.sin(0.7848 + 8328.691425 * t + 0.0001523 * t * t); //误差小于5%
    v -=
      179 * Math.sin(2.543 + 15542.7543 * t) + //误差小于0.3%
...
var touY = {
  ...
  MollCZ: function (W) {
    ...
    if (!B.length) {
      //创建插值表,当N为100时，精度为10^-7，在于80度的高度纬区精度降为10^-6，88度为10^-5
      for (i = 0; i < N; i++) {
        var y0 = 0,
          y = i / 100,
          c = pi_2 * sin(y * pi_2);
  ...
  toxy1: function (J, W, a) {
    ...
    var cosW = Math.cos(W),
      sinW = Math.sin(W);
    var L = Math.atan2(
      this.sinE0 * sinW - this.cosE0 * cosW * cosJ,
      sinJ * cosW
    );
  ...
  lineArr: function (d) {
    ...
    var lx,
      ly,
      dd = 0; //某条线的最后一个画线点(lx,ly)，dd为画线状态
    ...
    for (i = 0; i < d.length; i++) {
      ...
      i++; //在flash，这么写this.toxy(d[i++],d[i],G);不可正确运行
